...
class download():
    ...
    def get(self, url, timeout, proxy=None, num_retries=6, params=None):
        ...
        if proxy is None:  # 当代理为空时，不使用代理获取response
            try:
                ...
                if num_retries > 0:  # num_retries 限定重试次数
                    ...
                    print('downs', url)
                    return self.get(url, timeout, num_retries=num_retries-1, params=params)  # 调用自身,并将次数减1
                else:
                    print('downs', '开始使用代理')
                    time.sleep(10)
                    IP = ''.join(str(random.choice(self.iplist)).strip())
                    proxy = {'http': IP}
                    return self.get(url, timeout, proxy,params=params)

        else:
            try:
                return requests.get(url, headers=headers, proxies=proxy, timeout=timeout, params=params)  # 使用代理获取response
            except:
                ...
                if num_retries > 0:
                    ...
                    return self.get(url, timeout, proxy, num_retries-1, params)
                else:
