...
class mzitu():
    ...
    def mkdir(self, path): ##这个函数创建文件夹
        abspath = '/Users/admin/Desktop/splider/splider/mzitu/img_test'
        path = path.strip()
        isExists = os.path.exists(os.path.join(abspath, path))
        if not isExists:
            print(u'建了一个名字叫做', path, u'的文件夹！')
            os.makedirs(os.path.join(abspath, path))
            os.chdir(os.path.join(abspath, path)) ##切换到目录
            return True
        else:
